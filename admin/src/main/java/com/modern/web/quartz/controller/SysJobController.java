package com.modern.web.quartz.controller;
...
public class SysJobController extends BaseController
{
    ...
    @GetMapping("/export")
    public AjaxResult export(SysJob sysJob, @RequestParam(required = false, value = "ids")List<Long> ids)
    {
        List<SysJob> list = jobService.selectJobList(sysJob);
        if(ids != null && ids.size() > 0) {
            list = list.stream().filter(t -> ids.contains(t.getJobId())).collect(Collectors.toList());
        }
