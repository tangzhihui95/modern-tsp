...
public class SysPostController extends BaseController
{
    ...
    @GetMapping("/export")
    public AjaxResult export(SysPost post, @RequestParam(required = false, value = "ids")List<Long> ids)
    {
        List<SysPost> list = postService.selectPostList(post);
        if(ids != null && ids.size() > 0) {
            list = list.stream().filter(t -> ids.contains(t.getPostId())).collect(Collectors.toList());
        }
