...
import com.modern.web.domain.system.SysRoleDept;
import com.modern.web.domain.system.SysRoleMenu;
import com.modern.web.domain.system.SysUserRole;
import com.modern.web.mapper.system.SysRoleDeptMapper;
import com.modern.web.mapper.system.SysRoleMapper;
import com.modern.web.mapper.system.SysRoleMenuMapper;
...
public class SysRoleServiceImpl implements ISysRoleService
{
     ...
     */
    @Override
//    @Cacheable(value = "SysRole", key = "methodName + #userId")
     ...
     */
    @Override
    public List<SysRole> selectRolesByIds(Set<Long> roleIds) {
    ...
    @Override
    public List<SysRole> selectExportExcelList(SysRole role, List<Long> ids) {
        List<SysRole> list;
        if(ids != null && ids.size() > 0) {
            //通过选中条件筛选
            list = roleMapper.selectBatchIds(ids);
        } else {
            //通过查询条件查询
            list = selectRoleList(role);
        }
        for (SysRole sysRole : list) {
            List<String> roleKeys = roleMapper.getMenus(sysRole.getRoleId());
            sysRole.setRoleKey((roleKeys == null || roleKeys.size() == 0) ? "" : roleKeys.toString());
        }

        for (SysRole s : list) {
            List<TspVehicle> vehicles = xTspVehicleService.getAllVehiclesBySysRoleId(s.getRoleId());
