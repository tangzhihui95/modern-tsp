...
import com.modern.exInterface.repository.VehicleDriveMotorRepository;
...
public class VehicleDriveMotorService extends VehicleDriveMotorRepository {
    private final VehicleDriveMotorRepository vehicleDriveMotorRepository;
    private final VehicleDriveMotorMapper vehicleDriveMotorMapper;

    public PageInfo<VehicleDriveMotorParsedDTO> getPageList(VehicleSearchVO vo) {
        Wrapper<VehicleDriveMotor> ew = vehicleDriveMotorRepository.getPageListEw(vo);
        IPage<VehicleDriveMotor> pageList = vehicleDriveMotorMapper.getPageList(Page.of(vo.getPageNum(), vo.getPageSize()), ew);
        List<VehicleDriveMotorParsedDTO> dtoList = new ArrayList<>();
        for (VehicleDriveMotor record : pageList.getRecords()) {
            VehicleDriveMotorParsedDTO dto = VehicleDriveMotorParsedDTO.create(record);
            //TODO: dto == null
            dtoList.add(dto);
        }
    ...
    public AjaxResult export(VehicleSearchVO vo) {
        ...
        List<VehicleDriveMotorParsedDTO> list = getPageList(vo).getList();
        ...
        ExcelUtil<VehicleDriveMotorParsedDTO> util = new ExcelUtil<>(VehicleDriveMotorParsedDTO.class);
        return util.exportExcel(list, "_" + vo.getSearch() + "_驱动电机数据_" + vo.getCollectStartTime().format(LocalDateUtils.FORMAT_YYYYMMDDHHMMSS)
