...
import com.modern.common.utils.sql.SqlUtil;
import lombok.Data;
...
public class BaseController {
     ...
     * 设置请求分页数据
     */
    protected void buildPage(QueryBase queryBase) {
        Integer pageNum = queryBase.getPageNumber();
        Integer pageSize = queryBase.getPageSize();
        if (StringUtils.isNotNull(pageNum) && StringUtils.isNotNull(pageSize)) {
//            String orderBy = SqlUtil.escapeOrderBySql(pageDomain.getOrderBy());
//            Boolean reasonable = pageDomain.getReasonable();
     ...
     * @return
     */
    protected <T> FrontPageInfo<T> buildPageResult(List<T> list, QueryBase queryBase) {
        return FrontPageInfo.of(list, queryBase.getPageNumber(), queryBase.getPageSize(), new PageInfo(list).getTotal());
    }

    protected <T> FrontPageInfo<T> buildPageResult(QueryBase queryBase, PageExecutor<T> pe) {
	    List<T> list = pe.exec();
	    Page page = (Page) list;
	    return FrontPageInfo.of(pe.exec(), queryBase.getPageNumber(), queryBase.getPageSize(), new PageInfo(page).getTotal());
    }

    private interface PageExecutor<T> {
        List<T> exec();
    }
